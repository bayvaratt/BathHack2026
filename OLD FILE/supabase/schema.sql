...
CREATE TABLE destinations (
  ...
  city      text NOT NULL,
  country   text NOT NULL,
  region    text NOT NULL  -- e.g. 'Europe', 'Americas', 'Middle East'
);

-- ============================================
-- FLIGHT PRICES — one row per poll per flight per cabin class
-- checked = false until Friend 3's AI evaluates it
-- ============================================
CREATE TABLE flight_prices (
  ...
  stops          integer NOT NULL DEFAULT 0,  -- 0 = direct, 1 = 1 stop, 2 = 2 stops
  duration       text NOT NULL,               -- ISO 8601 e.g. 'PT8H30M'
...
-- ============================================
-- FLIGHT DETAILS — individual legs of a journey
-- direct flight  = 1 row  (segment_order = 1)
-- 1 stop flight  = 2 rows (segment_order = 1, 2)
-- 2 stop flight  = 3 rows (segment_order = 1, 2, 3)
-- ============================================
CREATE TABLE flight_details (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  flight_price_id  uuid NOT NULL REFERENCES flight_prices(id) ON DELETE CASCADE,
  segment_order    integer NOT NULL,   -- 1 = first leg, 2 = second leg, 3 = third leg
  flight_number    text NOT NULL,      -- e.g. 'BA234'
  flight_date      date NOT NULL,
  airline          text NOT NULL,
  origin           text NOT NULL,      -- departure airport for this leg
  destination      text NOT NULL,      -- arrival airport for this leg
...
-- updated by Friend 3's script after each poll
-- ============================================
...
-- ============================================
-- SUBSCRIBERS — email or WhatsApp, no login needed
-- must have at least one contact method
-- ============================================
CREATE TABLE subscribers (
  id           uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  email        text UNIQUE,
  phone_number text UNIQUE,   -- international format e.g. +447911123456
...
-- one row per origin + destination + cabin class combo
-- ============================================
...
-- ============================================
-- DEALS — discounts detected by Friend 3's AI
-- ============================================
CREATE TABLE deals (
  id               uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  origin           text NOT NULL REFERENCES origins(iata_code),
